...
function continueLogin(strategy, req, res, next) {
	passport.authenticate(strategy, async (err, userData, info) => {
		...
		if (userData.passwordExpiry && userData.passwordExpiry < Date.now()) {
			...
			await authenticationController.doLogin(req, userData.uid, remember);
			let destination;
			if (req.session.returnTo) {
...
authenticationController.doLogin = async function (req, uid, remember = false) {
	if (!uid) {
		return;
	}
	...
	jwtAuth.setAuthCookie(req.res, token, remember);

	// Also support traditional login for compatibility
